...
final class DefaultGroupsProvider implements DefaultGroupsProviderInterface
{
    ...
    public function getGroups()
    {
        ...
        $visitorName = $visitorsGroup->name[$this->contextLangId] ?? reset($visitorsGroup->name)
        ;
        $visitorsGroupDto = new DefaultGroup((int) $visitorsGroup->id, $visitorName);

        $groupsName = $guestsGroup->name[$this->contextLangId] ?? $guestsGroup->name
        ;
        $guestsGroupDto = new DefaultGroup((int) $guestsGroup->id, $groupsName);

        $customersName = $customersGroup->name[$this->contextLangId] ?? reset($customersGroup->name)
        ;
