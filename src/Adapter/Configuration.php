...
class Configuration extends ParameterBag implements ShopConfigurationInterface
{
    ...
    public function set($key, $value, ?ShopConstraint $shopConstraint = null, array $options = [])
    {
        ...
        $html = isset($options['html']) && (bool) $options['html'];
     ...
     */
    private function getLocalized($key, ?int $shopId = null, ?int $shopGroupId = null): array
    {
        $configuration = [];
    ...
    private function getShopGroupId(ShopConstraint $shopConstraint): ?int
    {
        ...
        if ($shopConstraint->getShopId() instanceof \PrestaShop\PrestaShop\Core\Domain\Shop\ValueObject\ShopId) {
            ...
            if (empty($shopGroupId)) {
                throw new ShopException(\sprintf('Shop group was not found for the shop with id %d.', $shopConstraint->getShopId()->getValue()));
            }
    ...
    public function deleteFromContext(string $key, ShopConstraint $shopConstraint): void
    {
        if ($shopConstraint->forAllShops()) {
            throw new ShopException(\sprintf('This method can not be used for all shops, if you want to completely delete a configuration use %s::remove() instead', static::class));
        }
