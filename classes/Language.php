...
class LanguageCore extends ObjectModel implements LanguageInterface
{
    ...
    private static function loadAllLanguagesDetails(): array
    {
        if (static::$_cache_all_languages_iso === null) {
            ...
            if (json_last_error() !== \JSON_ERROR_NONE) {
                throw new RuntimeException(sprintf('The legacy to standard locales JSON could not be decoded %s', json_last_error_msg()));
            }
        }
    ...
    public static function checkAndAddLanguage($iso_code, $lang_pack = false, $only_add = false, $params_lang = null)
    {
        ...
        if (isset($params_lang['allow_accented_chars_url']) && in_array($params_lang['allow_accented_chars_url'], ['1', 'true'], true)) {
    ...
    public static function installFirstLanguagePack(string $iso, array $params = [], array &$errors = []): bool
    {
        ...
        if (! Language::checkAndAddLanguage($iso, $lang_pack, false, $params)) {
            $errors[] = Context::getContext()->getTranslator()->trans('An error occurred while creating the language: %s', [$iso], 'Admin.International.Notification');
