...
class ProductCore extends ObjectModel
{
    ...
    public function getFieldsShop()
    {
        ...
        if ($this->update_fields === null || ! empty($this->update_fields['unity'])) {
    ...
    public function delete()
    {
        ...
        $id_shop_list = count($this->id_shop_list) > 0 ? $this->id_shop_list : Shop::getContextListShopID();
        ...
        foreach ($categories as $new_id_categ) {
            if (! in_array($new_id_categ, $current_categories, true)) {
                $product_cats[] = [
                    ...
                    'position' => $new_categ_pos[$new_id_categ],
                ];
            }
        }
    ...
    public function deleteAttachments($update_attachment_cache = true)
    {
        ...
        if ((bool) $update_attachment_cache) {
    ...
    {
        $all_shops = Context::getContext()->shop->getContext() === Shop::CONTEXT_ALL;
    ...
    public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, ?Context $context = null)
    {
        ...
        if (Group::isFeatureActive()) {
            ...
            JOIN `' . _DB_PREFIX_ . 'category_group` cg ON (cp.id_category = cg.id_category AND cg.`id_group` ' . (count($groups) > 0 ? 'IN (' . implode(',', $groups) . ')' : '=' . (int) Group::getCurrent()->id) . ')
    ...
    public static function getRandomSpecial($id_lang, $beginning = false, $ending = false, ?Context $context = null)
    {
        ...
        if ($product_reductions) {
            ...
            if ($ids_products !== '' && $ids_products !== '0') {
    ...
    public static function getPricesDrop(
        ...
        foreach ($ids_product as $product) {
            $productId = is_array($product) ? (int) $product['id_product'] : (int) $product;

            if (! in_array($productId, $tab_id_product, true)) {
    ...
    public static function displayWtPriceWithCurrency($params, &$smarty)
    {
        ...
        return $params['price'] !== null ? Tools::getContextLocale(Context::getContext())->formatPrice($params['price'], $currency) : null;
    }
    ...
    public static function sqlStock($product_alias, $product_attribute = null, $inner_join = false, ?Shop $shop = null)
    {
        ...
        return $sql . (StockAvailable::addSqlShopRestriction(null, $id_shop, 'stock') . ' )');
    }
    ...
    public function checkDefaultAttributes()
    {
        ...
        return (bool) ObjectModel::updateMultishopTable('Combination', ['default_on' => 1], 'a.id_product_attribute = ' . (int) $mini['id_attr']);
    }
    ...
    public static function duplicateAttributes($id_product_old, $id_product_new)
    {
        ...
        foreach ($result as $row) {
            ...
            foreach ($result3 as $row3) {
                ...
                if (in_array($current_supplier_key, $product_supplier_keys, true)) {
                    continue;
                }
    ...
    public static function getFrontFeaturesStatic($id_lang, $id_product)
    {
        ...
        if (! array_key_exists($id_product . '-' . $id_lang, self::$_frontFeaturesCache)) {
            $secondaryOrder = Configuration::get('PS_FEATURE_VALUES_ORDER') === 'name' ? 'fvl.value' : 'fv.position';
    ...
    public static function getAllCustomizedDatas($id_cart, $id_lang = null, $only_in_cart = true, $id_shop = null, $id_customization = null)
    {
        ...
        if (! $result = Db::getInstance()->executeS('
            ...
            ((int) $id_customization !== 0 ? ' AND cd.`id_customization` = ' . (int) $id_customization : '') . '
        ...
        if (! $result = Db::getInstance()->executeS(
            ...
            WHERE `id_cart` = ' . (int) $id_cart .
            ((int) $id_customization !== 0 ? ' AND `id_customization` = ' . (int) $id_customization : '') .
    ...
    protected function _deleteOldLabels()
    {
        ...
        if (
            ...
            && (! Db::getInstance()->execute(
                ...
                AND `' . _DB_PREFIX_ . 'customization_field`.`id_customization_field` >= ' . $customization_fields[Product::CUSTOMIZE_FILE][count($customization_fields[Product::CUSTOMIZE_FILE]) - $extra_file]
            ))
        ) {
            return false;
        }
        ...
        if (
            ...
            && (! Db::getInstance()->execute(
                ...
                AND `' . _DB_PREFIX_ . 'customization_field_lang`.`id_customization_field` = `' . _DB_PREFIX_ . 'customization_field`.`id_customization_field`
                AND `' . _DB_PREFIX_ . 'customization_field`.`id_customization_field` >= ' . $customization_fields[Product::CUSTOMIZE_TEXTFIELD][count($customization_fields[Product::CUSTOMIZE_TEXTFIELD]) - $extra_text]
            ))
        ) {
            return false;
        }
    ...
    public function getCustomizationFields($id_lang = false, $id_shop = null)
    {
        ...
        $context = Context::getContext();
        $front = isset($context->controller->controller_type) && $context->controller->controller_type === 'front';
        ...
        if ($this->deleteCategories() && ! empty($ids)) {
            ...
            foreach ($ids as $id) {
                ...
                if (array_key_exists((int) $id, $positions_lookup)) {
                    $pos = $positions_lookup[(int) $id];
                } elseif (array_key_exists((int) $id, $max_position_lookup)) {
                    $pos = $max_position_lookup[(int) $id] + 1;
                }
    ...
    public static function getAttributesParams($id_product, $id_product_attribute)
    {
        ...
        $cache_id = 'Product::getAttributesParams_' . (int) $id_product . '-' . (int) $id_product_attribute . '-' . $id_lang;
        ...
        if (! Cache::isStored($cache_id)) {
            ...
            LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_lang` pal
                ON (pal.`id_product_attribute` = pac.`id_product_attribute` AND pal.`id_lang` = ' . $id_lang . ')
    ...
    public function getProductType(): string
    {
        ...
        if (! empty($this->product_type) && in_array($this->product_type, ProductType::AVAILABLE_TYPES, true)) {
